  ...
  Future<Either<Failure, Auth>> login(LoginRequest loginRequest) async {
    ...
    if (await _networkInfo.isConnected) {
      ...
      if (response.status == 0) {
        //*success
        return Right(response.toDomain()); //? this will convert the response from the API to the domain model using the toDomain() method in the mapper class and return it to the presentation layer, and since we are using the Either type, we will return the data wrapped in a Right() which indicates that the operation was successful and the data is available.
      } else {
        //! failure
        return Left(
          Failure(code: 404, message: response.message ?? "API Error Message"), //? this will return a failure with the message from the API response, and since we are using the Either type, we will return the failure wrapped in a Left() which indicates that the operation was failed and the data is not available.
        );
      }
    } else {
      //? internet connection error
      return Left(Failure(code: 409, message: "no internet connection")); //? this will return a failure with a message "no internet connection", and since we are using the Either type, we will return the failure wrapped in a Left() which indicates that the operation was failed and the data is not available.
    }
  }
}
